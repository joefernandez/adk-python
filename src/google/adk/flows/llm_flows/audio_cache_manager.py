...
class AudioCacheManager:
  ...
  def cache_input_audio(
      self, invocation_context: InvocationContext, audio_blob: types.Blob
  ) -> None:
    ...
    audio_entry = RealtimeCacheEntry(
        role='user', data=audio_blob, timestamp=time.time()
    )
    ...
    logger.debug(
        ...
        len(audio_blob.data),
        len(invocation_context.input_realtime_cache),
    )
  ...
  def cache_output_audio(
    ...
    audio_entry = RealtimeCacheEntry(
        role='model', data=audio_blob, timestamp=time.time()
    )
    invocation_context.output_realtime_cache.append(audio_entry)

    logger.debug(
        'Cached output audio chunk: %d bytes, cache size: %d',
        len(audio_blob.data),
        len(invocation_context.output_realtime_cache),
    )
  ...
  async def flush_caches(
    ...
    {invocation_context.session.id}/adk_live/{filename}#{revision_id}"
  ...
  async def _flush_cache_to_services(
    ...
    try:
      ...
      artifact_ref = f'artifact://{invocation_context.app_name}/{invocation_context.user_id}/{invocation_context.session.id}/adk_live/{filename}#{revision_id}'
